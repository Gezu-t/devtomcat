...
intellij {
    ...
    // REMOVED: buildSearchableOptions = false (this causes the error)
    // Use the tasks block configuration instead
}
...
tasks {
    ...
    publishPlugin {
        ...
        channels.set(listOf(
            prop("pluginVersion").split('-').getOrElse(1) { "default" }.split('.').first()
        ))
    }
}