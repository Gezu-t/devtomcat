...
Two user-reported bugs surfaced after 1.0.10 shipped: a JDWP debug-launch crash on Java 8, and a JSTL `ClassNotFoundException` from an over-greedy classpath filter that survived `mvn clean install`. Both are root-cause fixes with regression coverage; no new features.

### Fixed
- **Debug launch on Java 8 failed with `JDWP Transport dt_socket failed to initialize, TRANSPORT_INIT(510)`.** The injected `-agentlib:jdwp` argument used the `address=*:port` wildcard host syntax, which was added in Java 9 (JDK-8041435). Java 8 and earlier reject the form and the JVM aborts before Tomcat boots. `TomcatJavaParametersBuilder.injectJdwpAgent` and `CatalinaScriptSupport.applyCustomScriptDebugSupport` now branch on the resolved JDK: Java 9+ keeps the explicit-wildcard `address=*:port` form (preserves the cross-host attach behaviour modern users rely on); Java 8 and earlier emit the no-host `address=port` form, which has the same effective binding coverage on Java 8 and is the only address syntax it accepts. Null / unparseable JDK versions default to the modern form for compatibility. New `supportsWildcardAddress` predicate is the shared decision point between the JVM-args path and the custom-script path. 10 regression tests cover Java 7 / Java 8 / Java 9 / Java 11 / Java 17 / Java 21, null JDK, unparseable version, and both code paths (`injectJdwpAgent` JVM args and the four-channel env-var injection in `applyCustomScriptDebugSupport`).
