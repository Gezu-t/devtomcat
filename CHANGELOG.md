...
- **`EcjJarSwapper.restoreBackup` could silently no-op when given a non-`.devtomcat-bak` path.** `stripBackupSuffix` returned the input unchanged when the suffix was missing, making `originalLocation` equal `backupPath`, which turned the atomic-move into a no-op while reporting success. Now refuses up front with a clear error naming the required suffix. Regression test `wrongSuffixIsRefused`.
- **Maven Central download didn't honour IntelliJ's HTTP proxy settings.** `EcjJarSwapper`'s downloader used raw `HttpURLConnection`, so corporate users behind a proxy couldn't use the swap action. Migrated to the platform's `com.intellij.util.io.HttpRequests` API which routes through user-configured proxies (with auth), the platform TLS trust store, and platform redirect handling.
- **ECJ swap notification fired every launch on affected Tomcats.** Inconsistent with the EOL warning's per-session dedup. Now keyed by ECJ JAR path + version so an unchanged install gets one balloon per IDE session; a manual ECJ change between launches re-fires.
- **JDK-mismatch quick-fix prompt fired on any blocking compatibility issue.** "No Tomcat server configured" is also a blocking issue; surfacing the prompt with title "JDK does not match Tomcat requirement" was misleading. Filtered to messages containing "Java" so only true JDK-related blocking issues trigger the prompt.
